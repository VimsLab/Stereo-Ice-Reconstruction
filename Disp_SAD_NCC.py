# Produces Disparity maps of images using SAD (OPT #1)or NCC (OPT #3) metric for user requested window size
...
def main():
    start = time.time()
    window_size = []
    img1 = cv2.imread('OATRC_L_Rect20.jpg')
    img2 = cv2.imread('OATRC_R_Rect20.jpg')
    h,w,c = img1.shape
    print('For image of size: ', h , 'x' , w)
    window = int(input('Enter window size: ' )) # 3
    s_range = int(input('Enter search range: ')) #32
    ...
    #cv2.waitKey(0)
    cv2.imwrite('/home/dasgupta/ICE/OATRC20SAD_Disp.jpg',l_rimg)
    #np.savetxt("disp_skq_40_ncc.txt", l_rimg, fmt="%4d", delimiter=",", newline="\n")
    end = time.time()
